        ...
        try:
            ...
            fields = {
                ...
                2: {
                    ...
                    32: 32768,
                    34: {
                        1: int "4382264208",  # Use the ACCOUNT'S UID here too!
                        2: 8,
                        3: bytes([15,6,21,8,10,11,19,12,17,4,14,20,7,2,1,5,16,3,13,18])
                    }
                },
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 2:
                                ...
                                try:
                                    ...
                                    # Create spam packet
                                    spam_packet = await Room_Spam(target_uid, room_id, "SB_GAMER", key, iv)
...
# IMPROVED TREE-STYLE HELP MENU SYSTEM (Commands in their original menus) 🌳
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = f"[b][c]{get_random_color()}Hey User Welcome To SBG BOT"
                            await safe_send_message(response.Data.chat_type, header, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Group Commands ─────
                            group_commands = """[C][B][FFD700]═══⚡ GROUP COMMANDS ⚡═══[00FFFF][B]
├─ [00FFFF]Create 3-Player Group
│  └─ [FF69B4]/3
├─ [00FFFF]Create 5-Player Group
│  └─ [FF69B4]/5
├─ [00FFFF]Create 6-Player Group
│  └─ [FF69B4]/6
├─ [00FFFF]Invite Player
│  └─ [FF69B4]/inv [uid]
├─ [00FFFF]Join Team
│  └─ [FF69B4]/join [team_code]
├─ [00FFFF]Leave Group
│  └─ [FF69B4]/exit
└─ [00FFFF]Start Match
   └─ [FF69B4]/start
...
├─ [FF1493]Spam Invites (30x)
│  └─ [BA55D3]/spm_inv [uid]
├─ [FF1493]Stop Spam Invites
│  └─ [BA55D3]/stop spm_inv
├─ [FF1493]Ghost Join Team
│  └─ [BA55D3]/ghost [code]
├─ [FF1493]Lag Attack Team
│  └─ [BA55D3]/lag [code]
├─ [FF1493]Stop Lag Attack
│  └─ [BA55D3]/stop lag
└─ [FF1493]Reject Spam
   └─ [BA55D3]/reject [uid]
[FF1493]━━━━━━━━━━━━[BA55D3]"""
...
├─ [FF6347]Send Evolution Emote
│  └─ [FFA500]/evo [uid] [1-21]
├─ [FF6347]Fast Evo (25x)
│  └─ [FFA500]/evo_fast [uid] [1-21]
├─ [FF6347]Custom Evo (X times)
│  └─ [FFA500]/evo_c [uid] [1-21] [x]
├─ [FF6347]Auto Cycle All Evo Emotes
│  └─ [FFA500]/random [uid]
└─ [FF6347]Stop Evo Emote Cycle
   └─ [FFA500]/sevos
[FF6347]━━━━━━━━━━━━[FFA500]"""
...
├─ [00CED1]Get player bio by uid
│  └─ [1E90FF]/bio [uid]
├─ [00CED1]Fetch Instagram User Info
│  └─ [1E90FF]/ig [username]
├─ [00CED1]Send custom spam message
│  └─ [1E90FF]/ms <text>
├─ [00CED1]Ask AI Anything
│  └─ [1E90FF]/ai [question]
├─ [00CED1]Admin Information
│  └─ [1E90FF]/admin
└─ [00CED1]Bot Status Check
   └─ [1E90FF]/status
[00CED1]━━━━━━━━━━━━[1E90FF]"""
                            ...
                            # ───── Badges Commands ─────
                            badge_commands = """[C][B][FF4500]═══⚡ BADGE JOIN REQUESTS ⚡═══[FF69B4][B]
├─ [FF69B4]Join Req Craftland Badge
│  └─ [FF4500]/s1 [uid]
├─ [FF69B4]Join Req New V-Badge
│  └─ [FF4500]/s2 [uid]
├─ [FF69B4]Join Req Moderator Badge
│  └─ [FF4500]/s3 [uid]
├─ [FF69B4]Join Req Small V-Badge
│  └─ [FF4500]/s4 [uid]
├─ [FF69B4]Join Req Pro Badge
│  └─ [FF4500]/s5 [uid]
└─ [FF69B4]Join Requests All Badge
   └─ [FF4500]/spam [uid]
[FF69B4]━━━━━━━━━━━━[FF4500]"""
                            ...
                            await asyncio.sleep(0.2)
                                                        ...
                                                        # ───── Auto Start Commands ─────
                            auto_commands = """[C][B][FF00FF]═══⚡ AUTO START BOT ⚡═══[FF1493][B]
├─ [FF1493]Start level up bot (24x7)
│  └─ [FF00FF]/lw [team_code]
└─ [FF1493]Stop level up Bot
   └─ [FF00FF]/stop_auto
[FF1493]━━━━━━━━━━━━[FF00FF]"""
                            await safe_send_message(response.Data.chat_type, auto_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
